...
AppHeader SHALL 不顯示任何頁面標題文字。標題列 SHALL 僅包含主題切換按鈕。

#### Scenario: 標題列無標題文字
- **WHEN** 使用者在任何頁面
- **THEN** 標題列 SHALL 僅顯示主題切換按鈕，左側不顯示任何標題文字
...
App SHALL 在 `src/assets/base.css` 中使用 CSS 自定義屬性 (custom properties) 定義所有視覺標記 (tokens)。`:root` 定義深色模式預設值；`body.light` 覆寫亮色模式數值。

標記類別 SHALL 包含：
- **品牌顏色**: `--color-primary`, `--color-primary-hover`, `--color-secondary`
...
- **THEN** `--color-bg-main` SHALL 解析為 `#1F1F1F`，`--color-bg-sub` 解析為 `#181818`

#### Scenario: 亮色模式變數值
- **WHEN** `body.light` 處於活動狀態
- **THEN** `--color-bg-main` SHALL 解析為 `#FFFFFF`，`--color-bg-sub` 解析為 `#F8F8F8`
